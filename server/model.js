...
function upvote(userId, postId, callback) {
    ...
    knex.raw('INSERT INTO upvoted (user_id, post_id) SELECT ?, ? WHERE NOT EXISTS (SELECT user_id, post_id FROM upvoted WHERE user_id = ? AND post_id = ?)', [userId, postId, userId, postId]).then(function() {
        callback();
    });
}
...
function removeUpvote(userId, postId, callback) {
    knex.raw('DELETE FROM upvoted WHERE user_id=? AND post_id=?', [userId, postId]).then(function() {
        callback()
    });
}
...
function commentUpvote(userId, commentId, callback) {
    knex.raw('INSERT INTO comment_upvoted (user_id, comment_id) SELECT ?, ? WHERE NOT EXISTS (SELECT user_id, comment_id FROM comment_upvoted WHERE user_id=? AND comment_id=?)', [userId, commentId, userId, commentId]).then(function() {
        callback();
    });
...
function removeCommentUpvote(userId, commentId, callback) {
    knex.raw('DELETE FROM comment_upvoted WHERE user_id=? AND comment_id=?', [userId, commentId]).then(function() {
        callback()
    });
}
...
function getPosts(userId, subpy, sortBy, ageWord, limit, callback) {
    ...
    if (userId === null) {
        query = 'SELECT ' + select + ', ' + ageSelection + ' as age FROM post WHERE post.subpy=? ' + maxAgeWhereClause + ' ORDER BY ? LIMIT ?';
        queryBindings = [subpy, orderBy, limit];
    } else {
        query = 'SELECT DISTINCT ON (post.id) ' + select + ', CASE WHEN user_id=? THEN true ELSE false END as upvoted, ' + ageSelection + ' as age FROM post LEFT OUTER JOIN upvoted ON post_id = post.id WHERE subpy=? ' + maxAgeWhereClause + ' ORDER BY post.id, upvoted DESC, ? LIMIT ?';
...
function getPostById(userId, postId, callback) {
    ...
    if (userId === null) {
        query = 'SELECT ' + select + ', false as upvoted FROM post WHERE post.id=?';
        bindings = [postId];
    } else {
        query = 'SELECT ' + select + ', (CASE WHEN user_id=? THEN true ELSE false END) as upvoted FROM post LEFT OUTER JOIN upvoted ON post_id=post.id WHERE post.id=? ORDER BY upvoted DESC LIMIT 1';
        bindings = [userId, postId];
    }
